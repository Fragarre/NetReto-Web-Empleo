...
def _denominacion(soup: BeautifulSoup, texto: str) -> str | None:
    # El h1 actual de sede.gva.es es el texto genérico "Navegación".
    # El <title> contiene la denominación real de la convocatoria.
    tag = soup.find("title")
    if tag:
...
def _tipo_convocatoria(texto: str) -> str | None:
    normal = _sin_acentos(texto)
    m = re.search(
        r"Informacion basica\s+Convocatoria\s+(.+?)\s+Prueba\s+",
        normal,
        re.I,
    )
    if not m:
...
def _estado(texto: str) -> str:
    normal = _sin_acentos(texto)
    if "plazo abierto" in normal or "plazo abierto" in normal or re.search(r"\bplazo abierto\b", normal):
        return "ABIERTO"
...
def _es_del_ambito(proceso: dict[str, Any]) -> bool:
    # Incluimos procesos de 2026/2027 y también convocatorias anteriores
    # que tengan una publicación/actualización efectiva en 2026 o posterior.
